...
Obstacle::Obstacle (GameEngine::Scene* parentScene, const irr::core::vector3df& position, const irr::core::vector3df size) : GameEngine::StaticPhysicalEntity(parentScene,0, "Obstacle")
{
  ...
  // 
	 ...
	 _node->setScale(size);
	// 
	 _node->setMaterialTexture(1, GameEngine::engine.getDevice()->getVideoDriver()->getTexture("textures/tex_cobble_1024_NRM.bmp"));
...
bool Obstacle::loadContent()
{
	...
	if(!_texture)
	{
		_texture = GameEngine::engine.getDevice()->getVideoDriver()->getTexture("textures/tex_cobble_d_1024.bmp");
		if(!_texture)
		{
			//TODO Crash bigtime here.
